  ...
  try {
    ...
    rooms.map((room) => {
      room.members.map((member) => {
        if (member.toHexString() !== user_id) connectedUsers.users.push(member);
      });
    });
